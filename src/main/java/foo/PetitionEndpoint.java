...
public class PetitionEndpoint {
	...
	public Entity addPetition(User user, Petition petition) throws UnauthorizedException {
		...
		try {
			Set<String> tags = new HashSet<>(Arrays.asList(petition.tags.split(",")));
	...
	public Entity addSignature(User user, Signature signature) throws UnauthorizedException {
		if (user == null) {
			throw new UnauthorizedException("Invalid credentials");
		}
		...
		Query query = new Query("Signature").setFilter(compositeFilter);
		...
		try {
        	...
        	Key petitionKey = KeyFactory.createKey("Petition", signature.pid);
